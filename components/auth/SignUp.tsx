...
export default function SignUp() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // signIn returns SignInResponse | undefined
      if (signInResult && signInResult.error) {
        // registration succeeded but auto signin failed — redirect to signin page
    ...
    <section className="min-h-screen flex flex-col items-center justify-center px-4">
      <div className="-mt-20">
        ...
        <div className="text-center mb-6">
          <h1 className={`text-4xl tracking-[0.2px] text-[#2e306a] font-bold ${londrina.className}`}>Create your account</h1>
          <p className="text-gray-600 text-md mt-2">
            Or{" "}
            <Link href="/auth/signin" className={`text-[#2e306a] font-medium hover:underline ${londrina.className}`}>
        ...
        <form onSubmit={handleSubmit} className="w-full max-w-sm space-y-4">
          ...
          >
            <span className={`${londrina.className}`}>{loading ? "Sending..." : "Send Otp"}</span>
